...
(t/deftest test-initialize-db
  (re-frame.test/run-test-sync
   ...
   (t/is {:you ::rps/rock
          :enemy ::rps/rock}
         @(re-frame/subscribe [::subs/you-and-enemy]))))
...
(t/deftest test-select-your-hand
  (re-frame.test/run-test-sync
   ...
   (t/testing "draw"
     ...
     (t/is "rock(YOU) VS rock(ENEMY)" @(re-frame/subscribe [::subs/you-and-enemy-hands]))
     (t/is ::rps/draw @(re-frame/subscribe [::subs/last-result]))
     (t/is "gray" @(re-frame/subscribe [::subs/result-color])))
   (t/testing "win"
     (re-frame/dispatch [::sut/next-game])
     (re-frame/dispatch [::sut/select-your-hand ::rps/paper])
     (t/is ::db/over @(re-frame/subscribe [::subs/scene]))
     (t/is "paper(YOU) VS rock(ENEMY)" @(re-frame/subscribe [::subs/you-and-enemy-hands]))
     (t/is ::rps/win @(re-frame/subscribe [::subs/last-result]))
     (t/is "red" @(re-frame/subscribe [::subs/result-color])))
   (t/testing "lose"
     (re-frame/dispatch [::sut/next-game])
     (re-frame/dispatch [::sut/select-your-hand ::rps/scissors])
     (t/is ::db/over @(re-frame/subscribe [::subs/scene]))
     (t/is "scissors(YOU) VS rock(ENEMY)" @(re-frame/subscribe [::subs/you-and-enemy-hands]))
     (t/is ::rps/lose @(re-frame/subscribe [::subs/last-result]))
