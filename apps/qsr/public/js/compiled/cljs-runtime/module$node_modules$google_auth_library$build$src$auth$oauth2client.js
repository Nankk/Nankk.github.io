...
"."+segments[1];signature=segments[2];try{envelope=JSON.parse(crypto.decodeBase64StringUtf8(segments[0]))}catch(err){throw err.message="Can't parse token envelope: "+segments[0]+"': "+err.message,err;}if(!envelope)throw Error("Can't parse token envelope: "+segments[0]);try{payload=JSON.parse(crypto.decodeBase64StringUtf8(segments[1]))}catch(err$17){throw err$17.message="Can't parse token payload '"+segments[0],err$17;}if(!payload)throw Error("Can't parse token payload: "+segments[1]);if(!certs.hasOwnProperty(envelope.kid))throw Error("No pem found for envelope: "+
